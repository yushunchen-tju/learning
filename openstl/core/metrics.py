...
def _prepare_weight(weight, H, W):
    """
    weight can be None, shape [H], or [H, W].
    returns weight flattened shape [HW] float64
    """
    if weight is None:
        return None
    w = np.asarray(weight, dtype=np.float64)
    if w.ndim == 1:
        assert w.shape[0] == H, f"weight(H,) expected H={H}, got {w.shape[0]}"
        w = w[:, None]  # [H,1]
        w = np.repeat(w, W, axis=1)  # [H,W]
    elif w.ndim == 2:
        assert w.shape == (H, W), f"weight(H,W) expected {(H,W)}, got {w.shape}"
    else:
        raise ValueError("weight must be None or shape [H] or [H,W]")
    return w.reshape(-1)


def ACC(pred, true, clim=None, weight=None, eps=1e-6):
    """
    Anomaly Correlation Coefficient (ACC), averaged over N,T,C.

    pred/true: [N, T, C, H, W]
    clim: climatology, broadcastable to [C, H, W] (or [1,1,C,H,W])
          if None -> use true.mean(axis=(0,1)) as approximate climatology.
    weight: optional spatial weight, shape [H] or [H,W] (e.g., cos(lat))
    """
    ...
    if clim is None:
        clim = true.mean(axis=(0, 1))  # [C,H,W]
    clim = np.asarray(clim, dtype=np.float64)
    # make it broadcastable to [N,T,C,H,W]
    if clim.ndim == 3:
        clim = clim[None, None, ...]
    elif clim.ndim == 5:
        pass
    else:
        raise ValueError("clim must be None, shape [C,H,W], or shape [N,T,C,H,W]")

    pa = pred - clim
    ta = true - clim

    w = _prepare_weight(weight, H, W)
    pa = pa.reshape(N, T, C, -1)  # [N,T,C,HW]
    ta = ta.reshape(N, T, C, -1)

    if w is None:
        num = np.sum(pa * ta, axis=-1)
        den = np.sqrt(np.sum(pa * pa, axis=-1) * np.sum(ta * ta, axis=-1)) + eps
    else:
        # weighted inner products over spatial points
        num = np.sum(pa * ta * w[None, None, None, :], axis=-1)
        den = np.sqrt(
            np.sum(pa * pa * w[None, None, None, :], axis=-1) *
            np.sum(ta * ta * w[None, None, None, :], axis=-1)
        ) + eps
...
def R2(pred, true, eps=1e-6):
    """
    Coefficient of Determination R^2, computed over all elements.
    pred/true: any same shape (typically [N,T,C,H,W])
    """
    pred = pred.astype(np.float64)
    true = true.astype(np.float64)
    ss_res = np.sum((pred - true) ** 2)
    true_mean = np.mean(true)
    ss_tot = np.sum((true - true_mean) ** 2) + eps
...
def metric(pred, true, mean=None, std=None, metrics=['mae', 'mse'],
           ...
           spatial_norm=False, return_log=True, threshold=74.0,clim=None, acc_weight=None):
    ...
    eval_res = {}
    eval_log = ""
    allowed_metrics = ['mae', 'mse', 'rmse', 'ssim', 'psnr','acc','r2', 'snr', 'lpips', 'pod', 'sucr', 'csi']
    ...
    if 'acc' in metrics:
        if channel_names is None:
            eval_res['acc'] = ACC(pred, true, clim=clim, weight=acc_weight)
        else:
            acc_sum = 0.
            for i, c_name in enumerate(channel_names):
                eval_res[f'acc_{str(c_name)}'] = ACC(
                    pred[:, :, i*c_width:(i+1)*c_width, ...],
                    true[:, :, i*c_width:(i+1)*c_width, ...],
                    clim=None if clim is None else clim[i*c_width:(i+1)*c_width, ...],
                    weight=acc_weight
                )
    ...
    if 'r2' in metrics:
        if channel_names is None:
            eval_res['r2'] = R2(pred, true)
        else:
            r2_sum = 0.
            for i, c_name in enumerate(channel_names):
                eval_res[f'r2_{str(c_name)}'] = R2(
                    pred[:, :, i*c_width:(i+1)*c_width, ...],
                    true[:, :, i*c_width:(i+1)*c_width, ...]
                )
                r2_sum += eval_res[f'r2_{str(c_name)}']
            eval_res['r2'] = r2_sum / c_group
